<template>
  <li class="relative hover:bg-gray-50">
    <inertia-link
      :href="proposal.route_proposal_view"
        ...
        <div v-if="proposal.has_recipient" class="flex flex-shrink-0 shadow-inner rounded-md mr-2">
          ...
          <div class="flex-1 hidden sm:flex items-center justify-between border-t border-r border-b border-white rounded-r-md truncate" style="min-width: 5rem; max-width: 8rem;">
            <div class="flex-1 pr-4 py-2 text-sm leading-5 truncate">
              <span class="text-gray-500 text-sm hover:text-gray-600 transition ease-in-out duration-150">{{ proposal.recipient_name }}</span>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    statusBadgeColor () {
      return this.proposal?.status_color ?? 'gray'
    }
  },
  data () {
    return {
