  ...
  <div class="relative bg-cool-gray-300 flex flex-col items-center" style="height: 65vh; border-top-left-radius: 50% 20%; border-top-right-radius: 50% 20%;">
    ...
    <span class="inline-flex rounded-md shadow-sm mt-4">
      <button
        ...
        :disabled="publicProposalMachineState.matches('unlocked.published.acceptingWithPayment')"
        :class="{ 'cursor-wait': publicProposalMachineState.matches('unlocked.published.acceptingWithPayment') }"
        ...
        <IconHeroiconsSpinner
          v-if="publicProposalMachineState.matches('unlocked.published.acceptingWithPayment')"
...
export default {
  props: {
    ...
    publicProposalMachineState: { type: Object, required: true },
  },
  components: {
    ContentEditable,
