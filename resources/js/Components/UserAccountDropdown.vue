  ...
  <div class="relative">
    <div>
      <button ref="menuTriggerButton" @click="handleAvatarClick" class="max-w-xs flex items-center text-sm rounded-full focus:outline-none focus:bg-primary-yellow-200 lg:p-2 lg:rounded-md lg:hover:bg-primary-yellow-200" aria-label="User menu" aria-haspopup="true">
        ...
        <svg class="hidden flex-shrink-0 ml-1 h-5 w-5 text-gray-400 lg:block" viewBox="0 0 20 20" fill="currentColor">
    ...
    <transition
      ...
      <div
        ...
        <div class="py-1 rounded-md bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
          <inertia-link @click="handleDropdownItemClick" :href="$route('profile.show')" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150" role="menuitem">User Settings</inertia-link>
          <inertia-link :href="$route('logout')" method="post" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition ease-in-out duration-150" role="menuitem">Logout</inertia-link>
        </div>
      </div>
    </transition>
