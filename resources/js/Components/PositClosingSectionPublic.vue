  ...
  <div class="relative bg-blue-gray-200 flex flex-col items-center" style="height: 65vh; border-top-left-radius: 50% 20%; border-top-right-radius: 50% 20%;">
    ...
    <template v-if="!proposal.is_accepted">
      ...
      <span class="inline-flex rounded-md shadow-sm mt-4">
        <button
          @click="$emit('accept-with-payment')"
          :disabled="publicPositMachineState.matches('unlocked.published.acceptingWithPayment')"
          :class="{ 'cursor-wait': publicPositMachineState.matches('unlocked.published.acceptingWithPayment') }"
          type="submit" class="bg-gray-800 border border-transparent rounded-md py-2 px-4 inline-flex justify-center text-sm leading-5 font-medium text-white hover:bg-gray-700 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray active:bg-gray-900 transition duration-150 ease-in-out">

          <IconHeroiconsSpinner
            v-if="publicPositMachineState.matches('unlocked.published.acceptingWithPayment')"
            class="h-5 w-5 mr-1.5"/>

          <IconHeroiconsSmallCheck v-else class="h-5 w-5 mr-1.5" />
    ...
    <PositAcceptedMeta v-if="proposal.is_accepted" />
...
export default {
  ...
  computed: {
    proposalTitleBadgeText () {
      return this.proposal?.name ?? 'Posit'
    }
  },
  watch: {
